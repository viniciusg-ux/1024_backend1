...
// Testes (igual aos que você fez nos outros exercícios)
console.log(`Pontos Y: ${resolveEquacao([1, 2, 3])}`)          
console.log(`Pontos Y: ${resolveEquacao([0, -1, 4])}`)         
console.log(`Pontos Y: ${resolveEquacao([5])}`)                
...
function encontrarMenorNumero(vetor: number[]): number {
    let menor = vetor[0]                     // começa com o primeiro número
...
// Testes (para você rodar e conferir)
...
function contaVogais(str: string): number {
    ...
    for (let i = 0; i < str.length; i++) {
        const letra = str[i];
        if (letra && vogais.includes(letra)) {
            contador++;
        }
    }
...
function contaConsoantes(str: string): number {
    ...
    for (let i = 0; i < str.length; i++) {
        const letra = str[i];
        if (letra && /[a-zA-Z]/.test(letra) && !vogais.includes(letra)) {
            contador++;
        }
    }
